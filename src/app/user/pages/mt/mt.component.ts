...
import { fromEvent, Subscription, forkJoin, interval, Subject } from 'rxjs';
...
export class MtComponent implements OnInit, OnDestroy {
  ...
  constructor(private service: ServiceService) {
    ...
    this.dateRange = { start: jan1, end: now };
  }
  ...
  startPolling() {
    interval(5000)
      ...
      .subscribe({
        next: ({ messages, users }) => {
          const updatedChats = this.buildChats(messages, users.users);
          this.mergeChats(updatedChats);
        },
  ...
  mergeChats(newChats: Chat[]) {
    const chatMap = new Map(this.chatList.map(chat => [chat.id, chat]));

    newChats.forEach(newChat => {
      const existing = chatMap.get(newChat.id);
      if (!existing) {
        this.chatList.unshift(newChat);
      } else {
        existing.messages = this.mergeMessages(existing.messages, newChat.messages);
        existing.lastMessage = newChat.lastMessage;
        existing.timestamp = newChat.timestamp;
      }
    });

    this.chatList.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  }

  mergeMessages(oldMsgs: Message[], newMsgs: Message[]): Message[] {
    const existing = new Set(oldMsgs.map(m => m.timestamp + m.text));
    const combined = [...oldMsgs];

    newMsgs.forEach(m => {
      if (!existing.has(m.timestamp + m.text)) {
        combined.push(m);
      }
    });
